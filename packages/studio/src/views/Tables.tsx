...
export function Tables({
  ...
  useEffect(() => {
    ...
    run(resource.id, resource.state, () =>
      ...
      .catch((err: unknown) =>
        setSchemaError(
          err instanceof api.ApiError ? err.message : "failed to load tables",
        ),
      );
  ...
  const loadRows = useCallback(
    (pageToLoad: number, filterToApply: string) => {
      ...
      const columnNames =
        table !== undefined ? table.columns.map((c) => c.name) : ["*"];
      const selectList =
        table !== undefined ? columnNames.map(quoteIdentifier).join(", ") : "*";
      ...
      run(resource.id, resource.state, () =>
        api.runQuery(resource.id, sql, [PAGE_SIZE, pageToLoad * PAGE_SIZE]),
      )
        .then((result) => {
          setRows(result.rows);
  ...
  function startEdit(
    ...
    if (currentTable === undefined || currentTable.primaryKey.length === 0)
      return;
    const text = currentValue === null ? "" : String(currentValue);
    setEditing({
      rowIndex,
      column,
      popout: text.length > POPOUT_AT || text.includes("\n"),
    });
    setEditValue(
  ...
  async function commitEdit(): Promise<void> {
    ...
    const whereSql = pk
      ...
      .join(" and ");
    const returningSql = currentTable.columns
    ...
    try {
      const result = await run(resource.id, resource.state, () =>
        api.runQuery(resource.id, sql, params),
      );
      ...
      if (updated !== undefined) {
        setRows((prev) =>
          prev === null
            ? prev
            : prev.map((r, i) => (i === editing.rowIndex ? updated : r)),
        );
      }
      setEditing(null);
      setSaveError(null);
      ...
      setSaveError(
        err instanceof api.ApiError ? err.message : "failed to save the edit",
      );
    }
  }
  ...
  return (
            ...
            {visibleTables.map((t) => (
              ...
              >
                ...
                {t.primaryKey.length === 0 && (
                  <span className="wb-table-note">no pk</span>
                )}
              </Button>
            ))}
          </div>
    ...
    >
      ...
      {schemaError !== null && (
        <div className="notice notice-danger">{schemaError}</div>
      )}
      {active === undefined ? (
        <div className="empty">
          <SpotRows />
        ...
        <>
          <div className="grid-toolbar">
            ...
            <div className="grid-toolbar-right">
              {queryMs !== null && (
                <span className="grid-timing">{queryMs}ms</span>
              )}
          ...
          {rowsError !== null && (
            <div className="notice notice-danger">{rowsError}</div>
          )}
          {saveError !== null && (
            <div className="notice notice-danger">{saveError}</div>
          )}
          {currentTable !== undefined &&
            currentTable.primaryKey.length === 0 && (
              <div className="hint-text">
                No primary key found on this table: edits are disabled to avoid
                an ambiguous update.
              </div>
            )}
                ...
                <thead>
                  <tr>
                    {Object.keys(rows[0] ?? {}).map((col) => {
                      const meta = currentTable?.columns.find(
                        (c) => c.name === col,
                      );
                      return (
                        <th key={col}>
                          ...
                          {meta !== undefined && (
                            <span className="col-type">{meta.dataType}</span>
                          )}
                      ...
                      {Object.entries(row).map(([col, value]) => {
                        ...
                        return (
                          <td
                            ...
                            onClick={() =>
                              !isEditing && startEdit(rowIndex, col, value)
                            }
                          >
  ...
  return (
    <div className="cell-popout" role="dialog" aria-label={`Edit ${column}`}>
      ...
      <div className="cell-popout-foot">
        <Button type="button" size="small" variant="ghost" onClick={onNull}>
          Set NULL
        </Button>
        <div className="row" style={{ marginLeft: "auto", gap: 6 }}>
          ...
          <Button type="button" size="small" variant="primary" onClick={onSave}>
            Save{" "}
