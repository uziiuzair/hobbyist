...
export function Schema({
  ...
  useEffect(() => {
    ...
    run(resource.id, resource.state, () => loadSchema((sql, params) => api.runQuery(resource.id, sql, params)))
      .then(setTables)
      .catch((err: unknown) => setError(err instanceof api.ApiError ? err.message : 'Could not read the schema'))
        ...
        <div className="schema-bar">
          ...
          <div className="schema-bar-right">
            {tables !== null && tables.length > 0 && (
              <CopyButton label="Copy schema" text={() => tables.map(tableDdl).join('\n\n')} />
            )}
            {active !== undefined && <CopyButton label="Copy table" text={() => tableDdl(active)} />}
          </div>
        </div>
      )}
              ...
              <Section title="Constraints" count={active.constraints.length}>
                {active.constraints.map((constraint) => (
                  <div className="ddl-line" key={constraint.name}>
                    <span className="ddl-kw">CONSTRAINT</span> <span className="ddl-name">{constraint.name}</span>{' '}
...
function ColumnLine({ column, table }: { column: ColumnInfo; table: TableInfo }) {
  const fk = table.foreignKeys.find((f) => f.column === column.name)
  const modifiers = columnModifiers(column)

  return (
    <div className="ddl-line">
      <span className="ddl-name">{column.name}</span> <span className="ddl-type">{column.dataType}</span>
